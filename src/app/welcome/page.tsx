...
function ActionStepsSection() {
  ...
  const steps = [
    ...
    { num: 2, title: "Questionnaire" },
    { num: 3, title: "Kickoff Call" },
  ];

  const StepIcon = ({ step, isActive }: { step: number; isActive: boolean }) => {
    const iconColor = isActive ? "text-[#b8926b]" : "text-[#a89f94]";
    const iconClass = `w-5 h-5 sm:w-6 sm:h-6 ${iconColor}`;
          ...
          {steps.map((step) => (
            ...
            >
              <div className="flex justify-center mb-1">
                <StepIcon step={step.num} isActive={activeStep === step.num} />
              </div>
